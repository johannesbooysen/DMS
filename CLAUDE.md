...
**Der Seed hat keine Dateien** — das ist richtig so (Binärdaten gehören nicht in ein SQL-Seed), fällt aber im Browser sofort auf: kaputte Seitenbilder, PDF-Download mit 404. Wer die Oberfläche ansieht, hält dann eine Lücke in den Testdaten für einen Fehler der Anwendung. `vorschau:befuellen` gibt deshalb **jedem** Beleg eine Datei, der Seiten behauptet — echte PDFs durch die **gewöhnliche** Aufbereitung, wie der Worker sie fährt. Der erste Entwurf rief nur die E2E-Vorbereitung auf; die deckt zwei Belege ab, weil die Tests zwei brauchen, und damit gingen beim ersten Rundgang zwei Drittel der Klicks ins Leere. Zum Schluss wird **ein Beleg gestempelt** — sonst muss man wissen, dass Stempel erst über das Postfach entstehen, und hält die halbe Anwendung für nicht vorhanden. Gestempelt wird über die Engine, nicht per `insert`: Hash-Kette, Layer und Aufgabenfortschritt sollen so entstehen wie im Betrieb.
